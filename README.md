...
This Project was forked from Josh Millers [ng-Boilerplate](http://joshdmiller.github.com/ng-boilerplate) and customized as [mvc-ng-boilerplate](https://github.com/gongin/mvc-ng-boilerplate/)
...
# Why MVC-Angular-Boilerplate

There are many projects that focus on getting front end support started quickly.  This project addresses those hesitant on using Angular because they work in .Net/Windows environments.
Just wiring Angular into a .Net application is a bit of a challenge and this project aims to simplify that.  Everything assumes you have Visual Studio 2012, however others
may learn from the src folder as it is plain angular.
This project template is to support front end (client-side) RESTful web application development using the **AngularJS** framework that makes API calls to MVC4 WebAPI protocols.
***

## Quick Start

Install Node.js, git for windows and then:

```cmd
> git clone https://github.com/gongin/mvc-ng-boilerplate Mvc.ApplicationName
> cd Mvc.ApplicationName
...
Finally, open `appDev/index.html` in your browser.

Happy hacking!

## Purpose

`mvcNgBoilerplate` is designed to make life easy by providing a basic framework
...
mvcNgBoilerplate also comes prepackaged with the most popular design frameworks
...
The principal goal of `mvcNgBoilerplate` is to set projects up for long-term
success.  So `mvcNgBoilerplate` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `mvcNgBoilerplate`.

But `mvcNgBoilerplate` is not an example of an AngularJS app: this is a
...
getting `mvcNgBoilerplate` up and running. Though `mvcNgBoilerplate` is really simple
...
`mvcNgBoilerplate` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/joshdmiller/mvc-ng-boilerplate my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `mvcNgBoilerplate`, like Twitter Bootstrap
...
Technically, `mvcNgBoilerplate` is now ready to go.
...
`mvcNgBoilerplate`. What follows in this section is a quick introduction to the
...
* `delta:less` - When any `*.less` file within `src/` changes, the
  ...
  `build/assets/mvc-ng-boilerplate.css`.
...
See the [issues list](http://github.com/gongin/mvc-ng-boilerplate/issues). And
