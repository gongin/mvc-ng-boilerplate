# The Tools Used in `mvcNgBoilerplate`

## Introduction

`mvcNgBoilerplate` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `mvcNgBoilerplate` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`mvcNgBoilerplate` uses git to manage its codebase. While in theory you don't have
to use Git once you download `mvcNgBoilerplate`, this project makes the assumption
...
While `mvcNgBoilerplate` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `mvcNgBoilerplate` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `mvcNgBoilerplate`; Grunt is
...
to get started with `mvcNgBoilerplate`. But for those curious or looking to go a
...
Grunt is the engine behind `mvcNgBoilerplate`. It's the magic that makes it move.
...
`mvcNgBoilerplate` comes with a `bower.json` file that looks something like this:

```js
{
  "name": "mvc-ng-boilerplate",
...
in `mvcNgBoilerplate` by default in there to see what I mean.
